...
def writeTopScorersToFile(file_id, doc_id, num_cands):
	...
	for x in range(num_cands):
		...
		if(is_valid_cand(ngram_string.split())):
  ...
  #expects ngram as array of strings
def is_valid_cand(ngram):
	#starts with stop word
	if(ngram[0] in stopwords): return False
	#ends in stop word
	if(ngram[-1] in stopwords): return False
	#first token all numbers

	if not (ngram[0].isalpha() or is_dimension_name(ngram[0])): return False
	return True
	
def is_dimension_name(token):
	reg = re.compile("\d[dD]")
	if (reg.match(token)): return True
	return False
...
def main():
	...
	printPath = "candidates/v600Candsv2/" + sys.argv[1] + "/"
